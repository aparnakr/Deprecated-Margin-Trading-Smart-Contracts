...
interface ComptrollerInterface {
    ...
    function mintAllowed(address cToken, address minter, uint mintAmount) external view returns (uint);
...
contract CToken is EIP20Interface, Exponential, TokenErrorReporter, ReentrancyGuard {
    ...
    function transferTokens(address spender, address src, address dst, uint amount) internal returns (uint) {
        // Check for under-collateralization
        uint allowed = comptroller.redeemAllowed(address(this), src, amount);
        if (allowed != 0) {
        ...
        (mathErr, srcTokensNew) = subUInt(accountTokens[src], amount);
        if (mathErr != MathError.NO_ERROR) {
            return fail(Error.MATH_ERROR, FailureInfo.TRANSFER_NOT_ENOUGH);
        }

        (mathErr, dstTokensNew) = addUInt(accountTokens[dst], amount);
        if (mathErr != MathError.NO_ERROR) {
     ...
     *  Since this produces a debugging trace for failures, it cannot be a 'view'.
    ...
    function borrowBalanceStoredInternal(address account) internal view returns (MathError, uint) {
        // XXX cleanup docs/spec here more
        ...
        /* recentBorrowBalance = borrowBalance * borrowIndex<stored> / borrowIndex<current> */
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        /* We emit a Redeem event, and a Transfer event */
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint) {
        ...
        /* We call the defense hook */
        comptroller.repayBorrowVerify(address(this), payer, borrower, vars.repayAmount);
